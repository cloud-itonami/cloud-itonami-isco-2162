(ns architecture.actor
  "LandscapeActor — the ISCO-08 2162 independent landscape architect actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one design operation request
  ...
  The unconditional invariant: the LandscapeAdvisor can never
  directly issue a stamped design, certify environmental compliance, or dispatch
  a robot action the LandscapeGovernor refuses — every
...
(defn build-graph
  "Build a compiled LandscapeActor graph. `store` implements
  `architecture.store/Store`. `advisor` implements
  `architecture.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one design operation request to completion or interrupt. `thread-id`
