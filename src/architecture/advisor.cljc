(ns architecture.advisor
  "LandscapeAdvisor — proposes a design operation (draft design concept,
  prepare specification, flag environmental concern, request client review)
  ...
  it never writes to the store, never issues a stamped design, and has no notion
  of project provenance or environmental-certification responsibility;
  `architecture.governor` is the independent system that decides whether the
  proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :draft-design-concept|:prepare-specification|:flag-environmental-compliance-issue|:request-client-review
     :effect :propose        ; the advisor NEVER issues stamped designs or certifies compliance
...
(def ^:private system-prompt
  "You are a landscape design advisor. Given a design operation
   request, propose an :op, an honest :confidence (0.0-1.0), and a
   :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. You never issue stamped designs or certify environmental compliance —
   those remain the licensed landscape architect's exclusive responsibility.")
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "design operation request: " (pr-str request))}]
